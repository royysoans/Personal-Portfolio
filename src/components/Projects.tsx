...
function ProjectCard({ project, compact }: { project: Project; compact?: boolean }) {
  return (
    <motion.article
      ...
      className="group flex flex-col overflow-hidden rounded-lg border border-black/[0.07] hover:border-black/[0.14] hover:shadow-md transition-all duration-300 h-full bg-surface"
    >
      <div className={`relative overflow-hidden bg-black/[0.04] border-b border-black/[0.06] ${compact ? 'aspect-[16/10]' : 'aspect-[16/9]'}`}>
        ...
        {project.featured && (
          <span className="absolute top-3 right-3 font-mono text-[9px] uppercase tracking-widest px-2 py-0.5 rounded-sm bg-surface/90 text-[#1A3B8B] border border-[#1A3B8B]/20">
            Featured
          </span>
        )}
      </div>
